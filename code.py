...
class ann:
    ...
    def forward_propogate(self):   # l is the list  passed  
        ...
        for i in range (len(self.layers)-1):
            ...
            self.activations.append(self.function[self.l[i]]( np.add( np.dot( self.weights[i],self.activations[i] ) , self.biases[i] )  ) )             # check and show this line 
    ...
    def backpropogation(self ):
        ...
        for i in reversed(range(len(self.len(self.layers) - 1))):
            self.error_wrt_z[i]=np.multiply(self.error_wrt_a[i],self.function_derivitive[self.l[i]](self.function[self.l[i]](self.z[i+1]))) 
            ...
            self.error_wrt_a[i-1]=np.dot (np.transpose(self.weights[i]), self.error_wrt_z[i])


    def train(self,epoches,alpha):
        for i in range(epoches):
            self.forward_propogate()
            self.backpropogation(self)
            for i in range(len (self.layers)-1):
